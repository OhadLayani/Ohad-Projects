...
int main()
{
	...
	//initManagerAndAirline(&manager, &company, AIRLINE_COMPRESS_FILE_NAME);
	...
	if (compress = 1) {
		saveAirlineCompressedToFile(&company, airlineFilename);
	}
	freeManager(&manager);
	freeCompany(&company);
...
int initManagerAndAirline(AirportManager* pManager, Airline* pCompany,const char* ailineFileName,int compress)
{
	...
	if (res == FROM_FILE) {
		return initAirlineFromFile(pCompany, pManager, ailineFileName, compress);
