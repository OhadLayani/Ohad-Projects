...
int		initAirlineFromFile(Airline* pComp, AirportManager* pManaer, const char* fileName, int CompressionStatus)
{
	...
	if (CompressionStatus == 1) {
		if (loadCompressedAirlineFromFile(pComp, pManaer, fileName)) {
			return initDateList(pComp);
		}
	}
...
int saveAirlineCompressedToFile(const Airline* pComp, const char* fileName)
{
	FILE* fp;
	fp = fopen(fileName, "wb");
	if (!fp) {
	...
	BYTE Airlinedata[2] = { 0 };
	
	int len = (int)strlen(pComp->name);
	Airlinedata[0] = pComp->flightCount>>1;
	Airlinedata[1]= ((pComp->flightCount)) << 7;
	Airlinedata[1] = Airlinedata[1] | pComp->sortOpt << 4 ;
	Airlinedata[1] = Airlinedata[1] | len &0xF	 ;
	

	if (fwrite(&Airlinedata, sizeof(BYTE), 2, fp) != 2)
	{
		fclose(fp);
		return 0;
	}
	...
	for (int i = 0; i < pComp->flightCount; i++) {
		SaveCompressedFlight(pComp->flightArr[i],fp);
	}

	return 1;
}

int loadCompressedAirlineFromFile(Airline* pComp, const AirportManager* pManager,
	const char* fileName) {
	FILE* fp;
	...
	BYTE	Airlinedata[2];
	if (fread(&Airlinedata, sizeof(BYTE), 2, fp) != 2)
	{
		fclose(fp);
		return 0;
	}
	pComp->flightCount = (Airlinedata[1] >> 7) | Airlinedata[0] << 1;
	pComp->sortOpt = (Airlinedata[1] >> 4) & 0x7;//change to 01110000
	int Airlinenamelen = Airlinedata[1] & 0xf;

	pComp->name = (char*)calloc(Airlinenamelen+1 ,sizeof(char));
	if (!pComp->name) {
		return 0;
	}
	if (fread(pComp->name, sizeof(char), Airlinenamelen, fp) != Airlinenamelen)
	{
		free(pComp->name);
		return 0;
	}
	...
	for (int i = 0; i < pComp->flightCount;i++) {
		...
		if (!loadCompressedFlightFromFile(pComp->flightArr[i], pManager, fp))
		{
			free(pComp->flightArr);
			fclose(fp);//ERRROR AIRLINE LEN PROBLEM?
			return 0;
		}
	}
